...
public interface MovieCinemaRepository extends JpaRepository<MovieCinema, Long> {
    ...
    List<MovieCinema> fetchAllAfterDate(@Param("dateTime") LocalDateTime dateTime);
    ...
    @Query(value = "SELECT * FROM movie_cinema mc JOIN cinema c" +
            "ON c.id= mc.cinema_id" +
            "JOIN location l " +
            "ON c.location_id = l.id WHERE l.name = ?1", nativeQuery = true)
    List<MovieCinema> findByLocationName(@Param("location") String name);
